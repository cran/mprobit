...
{ int m,i,j,nsim,ifail,m2,k,ksign;
  ...
  void mvn1(int, double [], double [], double [][M], int, double,
       double *, double *, int *);
  */
  void r_mvndu(int *, double *, double *, double *, int *,
       int *, double *, int *, double *);
  void r_mvndrh(int *, double *, double *, double *, int *, int *,
       int *, double *, int *, double *);
  ...
  { a=(double *) malloc((m+1) * sizeof(double));
    ...
    { printf("  k=%d\n", k);
      ksign=-k;
      r_mvndu(&m,a,b,rhmat,&ksign,&nsim,&eps,&ifail,&dera);
      ksign=k;
      r_mvndu(&m,a,b,rhmat,&ksign,&nsim,&eps,&ifail,&derb);
    ...
    for(j=1;j<m;j++)
    { for(k=j+1;k<=m;k++)
      { r_mvndrh(&m,a,b,rhmat,&j,&k,&nsim,&eps,&ifail,&drh);
...
void r_mvndu(int *m0, double *w, double *x, double *rhmat, int *ksign,
...
{ double der,tem,bb,pr,sd;
  ...
  */
  ...
  mvn1(m1,a,b,rc,*nsim,*eps,&pr,&sd,ifail);
...
void r_mvndrh(int *m0, double *w, double *x, double *rhmat, int *j0, int *k0,
...
{ double der,tem,pr,sd,rh,r1,rjm,rjm1;
  ...
  */
  ...
  if(a[m1]>-6. && a[m]>-6.)
  { mvn1(m2,ac,bc,rc,*nsim,*eps,&pr,&sd,ifail);
