mprobit.ar<- function(x, y, id, iprint=0, startpar=0)
{ if(!is.vector(x))
  ...
  out <- .C("mprobitar",as.double(x),as.integer(y),as.integer(id),
     as.integer(nrec),as.integer(npred),as.integer(nclbd),
     as.integer(nevals),as.integer(iprint),as.double(tol),
     th=as.double(th), nllkval=as.double(1),h=as.double(rep(0,np*np)) )
